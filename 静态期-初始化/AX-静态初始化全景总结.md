...
> 图中纵向箭头表示严格先后；缩进表示调用/创建关系。`from_vllm_config()` 是工厂入口，`Executor.get_class()` 只返回类，`AsyncLLM.__init__()` 才初始化实例；真正的 Executor 实例到 EngineCore 子进程中才创建。
